...
WHERE band_no = gave and performed_in = concert_no and band_contact = m_no and concert_venue = 'Royal Albert Hall' and con_date = (SELECT MAX (con_date) FROM concert WHERE concert_venue = 'Royal Albert Hall')) t1  inner join
(SELECT band_name as band2,m_name as conductor
FROM band,performance,musician,concert
WHERE conducted_by = m_no and gave = band_no and  performed_in = concert_no  and concert_venue = 'Royal Albert Hall' and con_date = (SELECT MAX (con_date) FROM concert WHERE concert_venue = 'Royal Albert Hall'))t2
on band_name = band2;
...
FROM musician,performer,band,plays_in
WHERE m_no = perf_is and perf_no = player and m_name = 'Sue Little')) t1
JOIN
(SELECT DISTINCT m_name,band_name as Jeff_band
FROM musician,performer,band,plays_in
WHERE band_no = band_id and m_name!= 'Jeff Dawn' and m_no = perf_is and perf_no = player and band_id in (SELECT band_id
FROM musician,performer,band,plays_in
WHERE m_no = perf_is and perf_no = player and m_name = 'Jeff Dawn')) t2
ON m_name = n1;